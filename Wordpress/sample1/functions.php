<?php
require_once 'libs/assets.php';
require_once 'libs/metaboxes.php';
require_once 'libs/plugins.php';
require_once 'libs/helpers.php';//
require_once 'libs/pins_management.php';//
...
$facebook_status    =   esc_html( get_option('samuiallvillas_facebook_login','') );
...
function samuiallvillas_admin_notice() {
    global $pagenow;
    global $typenow;
    ...
    if( esc_html( get_option('samuiallvillas_api_key') =='' ) ){
    ...
    if ( WP_MEMORY_LIMIT < 96 ) {
        print '<div class="error">
            <p>'.esc_html__( 'Wordpress Memory Limit is set to ', 'samuiallvillas' ).' '.WP_MEMORY_LIMIT.' '.esc_html__( 'Recommended memory limit should be at least 96MB. Please refer to : ','samuiallvillas').'<a href="http://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP" target="_blank">'.esc_html__('Increasing memory allocated to PHP','samuiallvillas').'</a></p>
        </div>';
    }
    ...
    if(PHP_VERSION_ID<50600){
        $version = explode('.', PHP_VERSION);
        print '<div class="error">
            <p>'.__( 'Your PHP version is ', 'samuiallvillas' ).' '.$version[0].'.'.$version[1].'.'.$version[2].'. We recommend upgrading the PHP version to at least 5.6.1. The upgrade should be done on your server by your hosting company. </p>
        </div>';
    }
    ...
    if( !extension_loaded('gd') && !function_exists('gd_info')){
        $version = explode('.', PHP_VERSION);
        print '<div class="error">
            <p>'.__( 'PHP GD library is NOT installed on your web server and because of that the theme will not be able to work with images. Please contact your hosting company in order to activate this library.','samuiallvillas').' </p>
        </div>';
    }
    ...
    if ( !extension_loaded('mbstring')) {
        print '<div class="error">
            <p>'.__( 'MbString extension not detected. Please contact your hosting provider in order to enable it.', 'samuiallvillas' ).'</p>
        </div>';
    }
    ...
    if (is_admin() &&   $pagenow=='post.php' && $typenow=='page' && basename( get_page_template($post))=='property_list_half.php' ){
        ...
        if ( $header_type != 5){
            print '<div class="error">
            <p>'.esc_html__( 'Half Map Template - make sure your page has the "media header type" set as google map ', 'samuiallvillas' ).'</p>
            </div>';
        }
    ...
    if (is_admin() &&   $pagenow=='edit-tags.php'  && $typenow=='estate_property') {
        ...
        print '<div class="error">
            <p>'.esc_html__( 'Please do not manually change the slugs when adding new terms. If you need to edit a term name copy the new name in the slug field also.', 'samuiallvillas' ).'</p>
        </div>';
    }
...
add_action( 'admin_notices', 'samuiallvillas_admin_notice' );


add_action('after_setup_theme', 'samuiallvillas_init');
if( !function_exists('samuiallvillas_init') ):
    function samuiallvillas_init() {
        ...
        load_theme_textdomain('samuiallvillas', get_template_directory() . '/languages');
        set_post_thumbnail_size(940, 198, true);
        add_editor_style();
        add_theme_support('post-thumbnails');
        add_theme_support('automatic-feed-links');
        add_theme_support('custom-background' );
        samuiallvillas_setup();
        add_action('widgets_init', 'register_samuiallvillas_widgets' );
        add_action('init', 'samuiallvillas_shortcodes');
        wp_oembed_add_provider('#https?://twitter.com/\#!/[a-z0-9_]{1,20}/status/\d+#i', 'https://api.twitter.com/1/statuses/oembed.json', true);
        samuiallvillas_image_size();
        add_filter('excerpt_length', 'samuiallvillas_excerpt_length');
        add_filter('excerpt_more', 'samuiallvillas_new_excerpt_more');
        add_action('tgmpa_register', 'samuiallvillas_required_plugins');
        add_action('wp_enqueue_scripts', 'samuiallvillas_scripts'); // function in css_js_include.php
        add_action('admin_enqueue_scripts', 'samuiallvillas_admin');// function in css_js_include.php
        update_option( 'image_default_link_type', 'file' );
    }
endif; // end   samuiallvillas_init
...
///////////////////////////////////////////////////////////////////////////////////////////
if (is_admin()) {
    add_action('admin_menu', 'samuiallvillas_manage_admin_menu');
}

if( !function_exists('samuiallvillas_manage_admin_menu') ):

    function samuiallvillas_manage_admin_menu() {
        global $theme_name;
        add_theme_page('SamuiallVillas Options', 'SamuiallVillas Options', 'administrator', 'libs/theme-admin.php', 'samuiallvillas_new_general_set' );
        add_theme_page('Import SamuiallVillas Themes', 'SamuiallVillas Import', 'administrator', 'libs/theme-import.php', 'samuiallvillas_new_import' );
...
endif; // end   samuiallvillas_manage_admin_menu
...
if( !function_exists('samuiallvillas_page_details') ):


function samuiallvillas_page_details($post_id){
    ...
    if($post_id !='' && !is_home() && !is_tax() ){
       ...
       $sidebar_status =  esc_html( get_post_meta($post_id, 'sidebar_option', true) );
        ...
        $sidebar_name   = esc_html( get_option('samuiallvillas_blog_sidebar_name', '') );
        $sidebar_status = esc_html( get_option('samuiallvillas_blog_sidebar', '') );
    }

    if(  'estate_agent' == get_post_type() && $sidebar_name=='' & $sidebar_status=='' ) {
        $sidebar_status = esc_html ( get_option('samuiallvillas_agent_sidebar','') );
        $sidebar_name   = esc_html ( get_option('samuiallvillas_agent_sidebar_name','') );
    }

    if($post_id !=''){
        if(  'estate_property' == get_post_type() &&  ($sidebar_status=='' || $sidebar_status=='global' )) {
            $sidebar_status = esc_html ( get_option('samuiallvillas_property_sidebar','') );
            $sidebar_name   = esc_html ( get_option('samuiallvillas_property_sidebar_name','') );
        }
    }
...
endif; // end   samuiallvillas_page_details
...
add_action('wp_head', 'samuiallvillas_generate_options_css');

if( !function_exists('samuiallvillas_generate_options_css') ):
function samuiallvillas_generate_options_css2() {
    $general_font   = esc_html( get_option('samuiallvillas_general_font', '') );
    $custom_css     = stripslashes  ( get_option('samuiallvillas_custom_css')  );
    $color_scheme   = esc_html( get_option('samuiallvillas_color_scheme', '') );
    echo "<style type='text/css'>" ;
    require_once ('libs/customcss.php');
    print htmlspecialchars_decode ($custom_css);
    samuiallvillas_custom_fonts_elements();
    echo  (  samuiallvillas_general_design_elements() );
    echo "</style>";
}

function samuiallvillas_generate_options_css() {
    $general_font   = esc_html( get_option('samuiallvillas_general_font', '') );
    $custom_css     = stripslashes  ( get_option('samuiallvillas_custom_css')  );
    $color_scheme   = esc_html( get_option('samuiallvillas_color_scheme', '') );
...
if (!function_exists('samuiallvillas_content_nav')) :

    function samuiallvillas_content_nav($html_id) {
        global $wp_query;

        if ($wp_query->max_num_pages > 1) :
            ?>
            <nav id="<?php echo esc_attr($html_id); ?>">
                <h3 class="assistive-text"><?php _e('Post navigation', 'samuiallvillas'); ?></h3>
                <div class="nav-previous"><?php next_posts_link(__('<span class="meta-nav">&larr;</span> Older posts', 'samuiallvillas')); ?></div>
                <div class="nav-next"><?php previous_posts_link(__('Newer posts <span class="meta-nav">&rarr;</span>', 'samuiallvillas')); ?></div>
            </nav><!-- #nav-above -->
        <?php
        endif;
    }

endif; // samuiallvillas_content_nav
...
if (!function_exists('samuiallvillas_comment')) :
    function samuiallvillas_comment($comment, $args, $depth) {
        ...
        switch ($comment->comment_type) :
            ...
            case 'trackback' :
                ...
                <li class="post pingback">
                    <p><?php _e('Pingback:', 'samuiallvillas'); ?> <?php comment_author_link(); ?><?php edit_comment_link(__('Edit', 'samuiallvillas'), '<span class="edit-link">', '</span>'); ?></p>
                <?php
                break;
                default :
                ?>
                ...
                <?php
                $avatar = samuiallvillas_get_avatar_url(get_avatar($comment, 55));
                print '<div class="blog_author_image singlepage" style="background-image: url(' . $avatar . ');">';
                comment_reply_link(array_merge($args, array('reply_text' => __('Reply', 'samuiallvillas'), 'depth' => $depth, 'max_depth' => $args['max_depth'])));
                print'</div>';
                ?>

                <div id="comment-<?php comment_ID(); ?>" class="comment">
                    <?php edit_comment_link(__('Edit', 'samuiallvillas'), '<span class="edit-link">', '</span>'); ?>
                    <div class="comment-meta">
                        <div class="comment-author vcard">
                            <?php
                            print '<div class="comment_name">' . get_comment_author_link().'</div>';
                            print '<span class="comment_date">'.__(' on ','samuiallvillas').' '. get_comment_date() . '</span>';
                            ?>
                        </div><!-- .comment-author .vcard -->

                    <?php if ($comment->comment_approved == '0') : ?>
                            <em class="comment-awaiting-moderation"><?php _e('Your comment is awaiting moderation.', 'samuiallvillas'); ?></em>
                            <br />
                    <?php endif; ?>
...
endif; // ends check for  samuiallvillas_comment
...
add_filter('user_contactmethods', 'samuiallvillas_modify_contact_methods');
if( !function_exists('samuiallvillas_modify_contact_methods') ):

function samuiallvillas_modify_contact_methods($profile_fields) {
...
endif; // end   samuiallvillas_modify_contact_methods
...
if( !current_user_can('activate_plugins') ) {
    function samuiallvillas_admin_bar_render() {
        global $wp_admin_bar;
        $wp_admin_bar->remove_menu('edit-profile', 'user-actions');
       }

    add_action( 'wp_before_admin_bar_render', 'samuiallvillas_admin_bar_render' );

    add_action( 'admin_init', 'samuiallvillas_stop_access_profile' );
    if( !function_exists('samuiallvillas_stop_access_profile') ):
    function samuiallvillas_stop_access_profile() {
        global $pagenow;

        if( defined('IS_PROFILE_PAGE') && IS_PROFILE_PAGE === true ) {
            wp_die( __('Please edit your profile page from site interface.','samuiallvillas') );
        }
    ...
    endif; // end   samuiallvillas_stop_access_profile
...
add_action( 'transition_post_status', 'samuiallvillas_correct_post_data',10,3 );

if( !function_exists('samuiallvillas_correct_post_data') ):

function samuiallvillas_correct_post_data( $strNewStatus,$strOldStatus,$post) {
    ...
    /* If this post is being published, try to restore the original author */
      if( $strNewStatus === 'publish' ) {
            ...
            if( $user->roles[0]=='subscriber'){
                ...
                samuiallvillas_select_email_type($user_email,'approved_listing',$arguments);

            }
    }
}
endif; // end   samuiallvillas_correct_post_data
...
add_action('get_header', 'samuiallvillas_my_filter_head');

if( !function_exists('samuiallvillas_my_filter_head') ):
    function samuiallvillas_my_filter_head() {
...
function hook_javascript(){
    ...
    if(isset($_GET['key']) && $_GET['action'] == "reset_pwd") {
        ...
        if(!empty($user_data)){
                ...
                if(!empty($reset_key) && !empty($user_data)) {
                        ...
                        $message = __('Your new password for the account at:','samuiallvillas') . "\r\n\r\n";
                        $message .= get_bloginfo('name') . "\r\n\r\n";
                        $message .= sprintf(__('Username: %s','samuiallvillas'), $user_login) . "\r\n\r\n";
                        $message .= sprintf(__('Password: %s','samuiallvillas'), $new_password) . "\r\n\r\n";
                        $message .= __('You can now login with your new password at: ','samuiallvillas') . get_option('siteurl')."/" . "\r\n\r\n";
                        ...
                        samuiallvillas_select_email_type($user_email,'password_reseted',$arguments);

                        $mess= '<div class="login-alert">'.__('A new password was sent via email!','samuiallvillas').'</div>';
  ...
  PRINT  $mes='<div class="login_alert_full">'.__('We have just sent you a new password. Please check your email!','samuiallvillas').'</div>';
...
function wpcf7_update_email_body($contact_form) {
    ...
    if ( $submission ){
        if( isset($postid) && get_post_type($postid) == 'estate_property' ){
            ...
            $mail['recipient']  = samuiallvillas_return_agent_email_listing($postid);
            $mail['body'] .= __('Message sent from page: ','samuiallvillas').get_permalink($postid);
...
function samuiallvillas_return_agent_email_listing($postid){
...
function tdd_tax_filter_posts_per_page( $value ) {
    $prop_no            =   intval( get_option('samuiallvillas_prop_no','') );
...
if( !function_exists('samuiallvillas_show_search_field_classic_form') ):
    function  samuiallvillas_show_search_field_classic_form($postion,$action_select_list,$categ_select_list ,$select_city_list,$select_area_list){
        ...
        if(isset($_GET['filter_search_action'][0]) && $_GET['filter_search_action'][0]!='' && $_GET['filter_search_action'][0]!='all'){
            ...
            $adv_actions_value=__('All Actions','samuiallvillas');
        ...
        if( isset($_GET['filter_search_type'][0]) && $_GET['filter_search_type'][0]!=''&& $_GET['filter_search_type'][0]!='all'  ){
            ...
            $adv_categ_value    = __('All Types','samuiallvillas');
        ...
        if(isset($_GET['advanced_city']) && $_GET['advanced_city']!='' && $_GET['advanced_city']!='all'){
            ...
            $advanced_city_value=__('All Cities','samuiallvillas');
        ...
        if(isset($_GET['advanced_area']) && $_GET['advanced_area']!=''&& $_GET['advanced_area']!='all'){
            ...
            $advanced_area_value=__('All Areas','samuiallvillas');
        ...
        $return_string.='
        <div class="col-md-3">
        <input type="text" id="'.$appendix.'adv_rooms" class="form-control" name="advanced_rooms"  placeholder="'.__('Type Bedrooms No.','samuiallvillas').'"
               value="';
        ...
        <div class="col-md-3">
        <input type="text" id="'.$appendix.'adv_bath"  class="form-control" name="advanced_bath"   placeholder="'.__('Type Bathrooms No.','samuiallvillas').'"
               value="';
        ...
        $show_slider_price      =   get_option('samuiallvillas_show_slider_price','');
        $where_currency         =   esc_html( get_option('samuiallvillas_where_currency_symbol', '') );
        $currency               =   esc_html( get_option('samuiallvillas_currency_symbol', '') );


        if ($show_slider_price==='yes'){
                $min_price_slider= ( floatval(get_option('samuiallvillas_show_slider_min_price','')) );
                $max_price_slider= ( floatval(get_option('samuiallvillas_show_slider_max_price','')) );
                ...
                $price_slider_label = samuiallvillas_show_price_label_slider($min_price_slider,$max_price_slider,$currency,$where_currency);

                $return_string.='<div class="col-md-6">
                <div class="adv_search_slider">
                    <p>
                        <label for="'.$ammount.'">'.__('Price range:','samuiallvillas').'</label>
                        <span id="'.$ammount.'"  style="border:0; color:#3C90BE; font-weight:bold;">'.$price_slider_label.'</span>
                    </p>
                    <div id="'.$slider.'"></div>';
                    $custom_fields = get_option( 'samuiallvillas_multi_curr', true);
        ...
        $return_string.='
            <div class="col-md-3">
                <input type="text" id="'.$price_low.'" class="form-control advanced_select" name="price_low"  placeholder="'.__('Type Min. Price','samuiallvillas').'" value=""/>
            </div>

            <div class="col-md-3">
                <input type="text" id="'.$price_max.'" class="form-control advanced_select" name="price_max"  placeholder="'.__('Type Max. Price','samuiallvillas').'" value=""/>
            </div>';
        }
...
add_filter( 'redirect_canonical','samuiallvillas_disable_redirect_canonical',10,2 );
function samuiallvillas_disable_redirect_canonical( $redirect_url ,$requested_url){
...
function samuiallvillas_file_upload_max_size() {
  static $max_size = -1;

  if ($max_size < 0) {
    // Start with post_max_size.
    $max_size = samuiallvillas_parse_size(ini_get('post_max_size'));

    // If upload_max_size is less, then reduce. Except if upload_max_size is
    // zero, which indicates no limit.
    $upload_max = samuiallvillas_parse_size(ini_get('upload_max_filesize'));
...
function samuiallvillas_parse_size($size) {
...
add_action('wp_head', 'samuiallvillas_rand654_add_css');
function samuiallvillas_rand654_add_css() {
    if ( is_singular('estate_property') ) {
        $local_id=get_the_ID();
        $samuiallvillas_global_page_template               =     intval( get_option('samuiallvillas_global_property_page_template') );
        $samuiallvillas_local_page_template                =     intval( get_post_meta($local_id, 'property_page_desing_local', true));
        if($samuiallvillas_global_page_template!=0 || $samuiallvillas_local_page_template!=0){

            if($samuiallvillas_local_page_template!=0){
                $id = $samuiallvillas_local_page_template;
            }else{
                $id = $samuiallvillas_global_page_template;
            }
...
if(!function_exists('samuiallvillas_all_prop_details_prop_unit')):
function samuiallvillas_all_prop_details_prop_unit(){
    ...
    $custom_fields = get_option( 'samuiallvillas_custom_fields', true);
    if( !empty($custom_fields)){
        $i=0;
        while($i< count($custom_fields) ){
            $name =   $custom_fields[$i][0];
            $slug         =     samuiallvillas_limit45(sanitize_title( $name ));
    ...
    $feature_list       =   esc_html( get_option('samuiallvillas_feature_list') );
    ...
    foreach($feature_list_array as $key => $value){
        ...
        $input_name =   samuiallvillas_limit45(sanitize_title( $post_var_name ));
...
function samuiallvillas_customtypo_scripts() {
    ...
    $h1_fontfamily  =   esc_html( get_option('samuiallvillas_h1_fontfamily', '') );
    $h1_fontsubset  =   esc_html ( get_option('samuiallvillas_h1_fontsubset','') );
    if($h1_fontsubset!=''){
    ...
    $h2_fontfamily  =   esc_html( get_option('samuiallvillas_h2_fontfamily', '') );
    $h2_fontsubset  =   esc_html ( get_option('samuiallvillas_h2_fontsubset','') );
    if($h2_fontsubset!=''){
    ...
    $h3_fontfamily  =   esc_html( get_option('samuiallvillas_h3_fontfamily', '') );
    $h3_fontsubset  =   esc_html ( get_option('samuiallvillas_h3_fontsubset','') );
    if($h3_fontsubset!=''){
    ...
    $h4_fontfamily  =   esc_html( get_option('samuiallvillas_h4_fontfamily', '') );
    $h4_fontsubset  =   esc_html ( get_option('samuiallvillas_h4_fontsubset','') );
    if($h4_fontsubset!=''){
    ...
    $h5_fontfamily  =   esc_html( get_option('samuiallvillas_h5_fontfamily', '') );
    $h5_fontsubset  =   esc_html ( get_option('samuiallvillas_h5_fontsubset','') );
    if($h5_fontsubset!=''){
    ...
    $h6_fontfamily  =   esc_html( get_option('samuiallvillas_h6_fontfamily', '') );
    $h6_fontsubset  =   esc_html ( get_option('samuiallvillas_h6_fontsubset','') );
    if($h6_fontsubset!=''){
    ...
    $p_fontfamily   =   esc_html( get_option('samuiallvillas_p_fontfamily', '') );
    $p_fontsubset   =   esc_html ( get_option('samuiallvillas_p_fontsubset','') );
    if($p_fontsubset!=''){
    ...
    $menu_fontfamily =  esc_html( get_option('samuiallvillas_menu_fontfamily', '') );
    $menu_fontsubset =  esc_html ( get_option('samuiallvillas_menu_fontsubset','') );
    if($menu_fontsubset!=''){
    ...
    foreach ($custom_fonts_array as $key=>$value){
        if($value!=''){
            ...
            wp_enqueue_style( 'samuiallvillas-custom-font'.$key, "$protocol://fonts.googleapis.com/css?family=$font$general_font_weight$custom_fonts_array_subset[$font]");
        }
    }


}
add_action( 'wp_enqueue_scripts', 'samuiallvillas_customtypo_scripts' );





function samuiallvillas_search_delete_user( $user_id ) {
	global $wpdb;
        ...
        $args = array(
        'post_type'        => 'samuiallvillas_search',
        'post_status'      =>  'any',
        'posts_per_page'   => -1 ,
        'meta_query' => array(
		array(
...
add_action( 'delete_user', 'samuiallvillas_search_delete_user' );
...
if(!function_exists('samuiallvillas_add_meta_post_to_search')):
function samuiallvillas_add_meta_post_to_search($meta_array){
    global $table_prefix;
    ...
    foreach($meta_array as $key=> $value){
        ...
        switch ($value['compare']) {
            case '=':
                $potential_ids[$key]=samuiallvillas_get_ids_by_query("
                    SELECT post_id
...
$remove_script_version= get_option('samuiallvillas_remove_script_version','');
